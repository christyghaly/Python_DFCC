...
def interpolate_flow_fields(u_arrays, v_arrays):
    ...
    x_axis = numpy.arange(u_arrays[0].shape[0]) #from 0 to 287
    y_axis = numpy.arange(u_arrays[0].shape[1]) # from 0 to 287
    ...
    for t in tqdm(range(len(u_arrays)), bar_format='{l_bar}{bar:50}{r_bar}{bar:-50b}'):
        ...
        print(u_arrays[t])
        print(v_arrays[t])
        znew_u = fu(x_axis, y_axis)
        plt.plot(x_axis, znew_u[0, :], 'b-')
        plt.show()
...
####################################################################################################
def compute_trajectories(frame, fu_arrays, fv_arrays, pixel_threshold=15, parallel=False, usePixelThreshold=True):
